 ...
 * DEPT_PJ is always pinned to their own session department. SUPER_ADMIN
 * must pass ?departmentId=; the id is validated against active,
 * candidate-owning departments so an arbitrary/foreign UUID cannot be probed.
 */
export async function resolveRequestDepartmentId(
  ...
  if (context.role === "SUPER_ADMIN" && !(await isCandidateOwningDepartment(departmentId))) {
