...
import { adminResetSelection, SelectionDecisionError } from "@/server/candidates/selection";
...
export async function POST(request: Request, routeContext: RouteContext) {
  try {
    ...
    if (!parsed.success) {
      return noStoreJson({ data: null, error: { code: "VALIDATION_ERROR", message: "Alasan reset wajib diisi.", fields: parsed.errors } }, 400);
    }

    const result = await adminResetSelection({
      candidateId: id,
